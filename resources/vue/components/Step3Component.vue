<template>
  ...
  <div class="w-full max-w-2xl mx-auto flex flex-col gap-10 mt-10">
    <div>
    ...
    <div>
      <div class="toggle-detail relative flex items-center cursor-pointer select-none bg-paleblue text-center text-[18px] font-semibold min-h-[48px]" @click="toggleDetail = !toggleDetail">
      ...
      <div class="toggle-detail-content flex flex-col gap-6 md:gap-10 mt-5" v-show="toggleDetail">
        <div>
          <label class="flex items-center gap-2 mb-3">
...
export default {
  ...
  data() {
    return {
      ...
      households: ['1人', '2人', '3人', '4人', '5人以上'],
      errors: {}
    };
  },
  methods: {
    onCheck(val, e) {
      let arr = [...this.equipment];
      if (e.target.checked) {
      ...
      this.$emit('update:equipment', arr);
    },
    validate() {
      let valid = true;
      const errs = {};
      if (!this.company) {
        errs.company = 'ガス会社名を入力してください';
        valid = false;
      }
      this.errors = errs;
      return valid;

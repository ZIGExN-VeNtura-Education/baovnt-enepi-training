<template>
  ...
  <div class="w-full max-w-2xl mx-auto flex flex-col gap-4 px-4" @submit.prevent="onSubmit">
    ...
    <p class="text-xs text-center my-10">
      個人情報の取扱いについては
      <span class="text-lightblue cursor-pointer" @click="$emit('show-terms')">利用規約</span>
      および
      <span class="text-lightblue js-terms-link cursor-pointer" @click="$emit('show-privacy')">プライバシーポリシー</span>
      に同意してください
    </p>
...
const emit = defineEmits(['update:name', 'update:furigana', 'update:phone', 'update:email', 'prev', 'submit', 'show-terms', 'show-privacy']);
function onSubmit() {
  emit('submit');
}
</script> 
